...
/// Thin executable entry point. All command logic lives in `DiscFreeCLI`.
///
/// This dispatches to the parsed command's async `run()` explicitly rather than calling
/// `DiscFree.main()`: in a top-level `main.swift` the compiler resolves `main()` to the
...
enum DiscFreeMain {
    static func main() async {
        do {
            var command = try DiscFree.parseAsRoot()
